...
def plotCorpusRhyme():
    ...
    #Assigns labels, plots 2-tuples as red circles ('ro') then centers the plot
...
def printComparisonMatrix():
    ...
    for x in range(comparableParameter):
        #As explained above: poem at [x] has its translated version at [x+comparableParameter]
        ...
        #Uses a string to put in one line all the computes of the poem, then formats it and its added to the matrix (comparisonMatrix)
...
#Plots the variation of a single-poem creativity by modifying the rhyme parameter
#Takes as argument the name of the poem (poemName) and the maximum variable rhyme parameter (It'll plot from 1 to that value)
def singleRhymeParameterComparisonCosine(poemName, number):
    
    #Creates a vector of rhyme comparisons, each position represents a comparison with a different parameter
    #being [position 0] the comparison using "1" as rhyme parameter and position [number] the comparison using the parameter given by the user
    rhymeComparisonByCosine = []
    #Loops from 1 to the value given calculating the cosine distance of the poem and its translation for each parameter,
    #thens appends the result in the vector defined above
    for i in range(1, number+1):
        A = poemVector(poemName,i)
        B = poemVector(dict[poemName],i)
        rhymeComparisonByCosine.append(cosine_distance(A,B))

    #Calculates plot limits so it'd be clear to see the effect vary the rhyme parameter does, this also centers the plot
    ...
    #Creates an auxiliar vector to be able to automatically format plot distribution
    aid = [0] * number
    for i in range(number):
        aid[i]=i+1

    #Assigns labels, plots the results previously obtained as dots linked by a line then sets the plot limits
    import matplotlib.pyplot as plt
    plt.title('"' + poemName + '" creativy when varying rhyme parameter')
    plt.ylabel('Creativity by cosine')
    plt.xlabel('Rhyme')
    plt.plot(aid,rhymeComparisonByCosine,'-o')
    plt.axis([0, number+1, lowerPlotLimit, upperPlotLimit])
    plt.show()

#Plots the variation of a the full corpus creativity (poem by poem) by modifying the rhyme parameter
#It only takes as parameter the maximum value of the rhyme parameter (It'll plot from 1 to the given value)
def corpusRhymeParameterComparisonCosine(numberGivenByTheUser):
    
    #Creates a vector of rhyme comparisons, each position represents a comparison with a different parameter
    #being [position 0] the comparison using "1" as rhyme parameter and position [number] the comparison using the parameter given by the user
    #This vector will have a length of = numberGivenByUser*numberOfPoemsInCorpus and each poem will have associated a "numberGivenByTheUser" positions
    #For example if the user gives "5" as the maximum parameter and the corpus is composed by 3 poems then:
    #   Poem 1 will have associated positions [0] to [4]
    #   Poem 2 will have associated positions [5] to [9]
    #   Poem 3 will have associated positions [10] to [14]
    #Being positions [0],[5] and [10] the comparisons using parameter "1" and positions [4],[9] and [14] the comparisons using parameter "numberGivenByTheUser"
    rhymeComparisonByCosine = []
    
    #Auxiliar vectors used to compute the maximum and minimum values of each comparison
    lows = []
    ups = []

    #Through a double loop iterate trough the corpus calculating creativity using cosine distance
    #then appends the maximum and minimum values of each poem to the auxiliar vectors (lows and ups) 
    for key, value in dict.items():
        for i in range(1, numberGivenByTheUser+1):
            A = poemVector(key,i)
            B = poemVector(dict[key],i)
            rhymeComparisonByCosine.append(cosine_distance(A,B))
        lows.append(min(rhymeComparisonByCosine))
        ups.append(max(rhymeComparisonByCosine))

    #Takes the minimun of all minimum values and the maximum of all maximum values and uses them to compute plot limits so it'd be centered
    lowerPlotLimit = min(lows) - (min(lows)*1)/100
    upperPlotLimit = max(ups) + (max(ups)*1)/100

    #Creates an auxiliar vector to be able to automatically format plot distribution
    aid = [0] * numberGivenByTheUser
    for i in range(numberGivenByTheUser):
        aid[i]=i+1
    
    #Assigns labels
    import matplotlib.pyplot as plt2
    plt2.title('Corpus creativy when varying rhyme parameter')
    plt2.ylabel('Creativity by cosine')
    plt2.xlabel('Rhyme')

    #Plots the results previously obtained as dots linked by a line then sets the plot limits
    #Through this loops, it's able to distinguish each poem by its corresponding intervals (Explained above)
    for n in range(len(dict)):
        n *= numberGivenByTheUser
        plt2.plot(aid,rhymeComparisonByCosine[(0+n):(numberGivenByTheUser+n)],'-o')
    plt2.axis([0, numberGivenByTheUser+1, lowerPlotLimit, upperPlotLimit])
    plt2.show()

#Demo
printComparisonMatrix()
varyRhyme = int(input("Maximum value of rhyme parameter: "))
singleRhymeParameterComparisonCosine("One's Self I Sing", varyRhyme)
corpusRhymeParameterComparisonCosine(varyRhyme)
plotCorpusRhyme()
